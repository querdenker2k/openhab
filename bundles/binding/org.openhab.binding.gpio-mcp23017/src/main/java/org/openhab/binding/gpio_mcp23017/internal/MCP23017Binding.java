...
public class MCP23017Binding extends AbstractActiveBinding<MCP23017BindingProvider>implements ManagedService {
    ...
    private Map<String, MCP23017Device> deviceMap = new HashMap<>();
    ...
    public void deactivate() {
        for (MCP23017Device device : this.deviceMap.values()) {
            device.stopPolling();
        }
    }
    ...
    public void updated(Dictionary<String, ?> properties) throws ConfigurationException {
        if (properties != null) {
            ...
            Map<String, MCP23017Config> configMap = new HashMap<>();
            while (keys.hasMoreElements()) {
                ...
                if (!key.equals("service.pid")) {
                    ...
                    if (!configMap.containsKey(id)) {
                        configMap.put(id, new MCP23017Config(id));
                    }
            ...
            for (String id : configMap.keySet()) {
                MCP23017Config mcp23017Config = configMap.get(id);
                logger.debug("******************");
                logger.debug("id: " + mcp23017Config.getId());
                logger.debug("address: " + String.format("%02x", mcp23017Config.getAddress()));
                logger.debug("pollInterval: " + mcp23017Config.getPollInterval());
                logger.debug("******************");

                try {
                    this.deviceMap.put(id,
                            (MCP23017Device) this.gpioLoader.createI2CDevice(mcp23017Config, MCP23017Device.class));
